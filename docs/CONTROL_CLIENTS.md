# Socktainer control clients

Socktainer has two local control clients:

- `SocktainerMenu` is a native SwiftUI menu-bar app.
- `raycast/` is a TypeScript and React extension that uses Raycast UI.

Both clients use the `SocktainerControl` module. Raycast uses the module through
the versioned JSON output of `socktainerctl`. The clients do not build shell
...
- locks one engine state at `/private/var/tmp/socktainer-<uid>/engine`;
- creates `$HOME/.socktainer/container.sock`;
- optionally creates the `socktainer` Docker context;
...
The package installer puts the daemon and helpers in `/opt/socktainer`. Homebrew
...
`SocktainerControl` is the deep module at the control seam. Its small interface
is:
...
The module uses a per-user LaunchAgent named `io.github.socktainer.daemon`.
...
```console
socktainerctl status --json
socktainerctl support-report --json
socktainerctl daemon start --json
socktainerctl daemon stop --json
socktainerctl daemon restart --json
socktainerctl containers list --json
socktainerctl containers start CONTAINER --json
socktainerctl containers stop CONTAINER --json
socktainerctl logs daemon --json
socktainerctl logs container CONTAINER --json
```
...
in place and return to the list. Raycast exposes one **Socktainer** command with
...
```console
make menu-app
open .build/debug/Socktainer.app
```

Run the process-level status-item popover check:

```console
scripts/test-menu-popover.sh .build/debug/Socktainer.app/Contents/MacOS/SocktainerMenu
```
...
For a source build, set the Raycast `socktainerctl Executable` preference to the
absolute path of `.build/debug/socktainerctl`. An installed package is found at
`/opt/socktainer/bin/socktainerctl`, `/opt/homebrew/bin/socktainerctl`, or
...
owner. The extension therefore calls `socktainerctl` with an executable path and
...
1. Put the daemon, VMM artifacts, `socktainerctl`, and launch-agent property list
   in a signed `Socktainer.app` bundle.
...
`raycast/` subtree now has a separate MIT license. The main Socktainer
