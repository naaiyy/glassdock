...
Socktainer Menu is a direct-distribution macOS app. It must use a `Developer ID
...
until its effect on the user's Socktainer Unix socket is tested. App Sandbox is
...
codesign --verify --deep --strict --verbose=4 Socktainer.app
codesign --display --verbose=4 --entitlements :- Socktainer.app
spctl --assess --type execute --verbose=4 Socktainer.app
```
...
requirements. If Socktainer later uses App Store Connect, it must provide
...
the local `socktainerctl` requirement, and states the data behavior. Confirm
...
open .build/release/Socktainer.app
```
...
each Socktainer command, capture the window, select **Save to Metadata**, and
