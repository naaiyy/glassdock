...
help:
	...
	@echo "  control          - Build socktainerctl"
	@echo "  menu-app         - Build a local Socktainer.app bundle"
...
.PHONY: control
control:
	@$(SWIFT) build -c $(BUILD_CONFIGURATION) --product socktainerctl
...
menu-popover-test: menu-app
	@bash scripts/test-menu-popover.sh .build/$(BUILD_CONFIGURATION)/Socktainer.app/Contents/MacOS/SocktainerMenu
...
publishing-validate:
	@plutil -lint \
		Apps/SocktainerMenu/Info.plist \
		Apps/SocktainerMenu/SocktainerMenu.entitlements \
		Apps/SocktainerMenu/PrivacyInfo.xcprivacy >/dev/null
	@jq -e . Apps/SocktainerMenu/Distribution.json raycast/package.json >/dev/null
	@codesign --verify --deep --strict .build/release/Socktainer.app
	@unzip -tq ".build/release-distribution/Socktainer-$(APP_RELEASE_VERSION)-macOS-arm64.zip" >/dev/null
