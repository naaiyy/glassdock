...
private final class AppKitStatusPopoverPresenter: NSObject, StatusPopoverPresenting, NSPopoverDelegate {
    ...
    override init() {
        ...
        if let button = statusItem.button {
            let image = NSImage(systemSymbolName: "shippingbox.fill", accessibilityDescription: "Socktainer")
            image?.isTemplate = true
            button.image = image
            button.imagePosition = .imageOnly
            button.toolTip = "Socktainer"
            button.setAccessibilityLabel("Socktainer menu")
            button.target = self
