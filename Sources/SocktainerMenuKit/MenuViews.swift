    ...
    var body: some View {
        HStack(spacing: 11) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("Socktainer \(model.statusLabel)")
                    .font(.headline)
                Text(subtitle)
                    .font(.caption)
            ...
            if model.isLoading {
                ProgressView()
                    .controlSize(.small)
                    .accessibilityLabel("Refreshing Socktainer")
            }
            ...
            .help("Refresh")
            .accessibilityLabel("Refresh Socktainer")
        }
    }
            ...
            Group {
                if model.snapshot == nil {
                    ...
                    ContentUnavailableView(
                        ...
                        description: Text(model.snapshot?.daemon.message ?? "Socktainer is not available.")
                    )
        ...
        if let snapshot = model.snapshot {
            ...
            case .managedLaunchAgent:
                if snapshot.daemon.state == .stopped {
                    Button("Start Socktainer") { Task { await model.perform(.startDaemon) } }
                } else {
