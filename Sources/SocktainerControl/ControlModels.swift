...
public enum ControlError: LocalizedError, Sendable, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .daemonExecutableNotFound:
            return "The socktainer daemon executable was not found."
        case .invalidSocketPath(let path):
            return "The Unix socket path is invalid: \(path)"
        case .socket(let message):
            return "Cannot connect to the Socktainer socket: \(message)"
        case .malformedResponse(let message):
            return "Socktainer returned an invalid response: \(message)"
        case .requestFailed(let status, let message):
            return "Socktainer request failed with status \(status): \(message)"
        case .lifecycle(let message):
            return "Cannot control the Socktainer daemon: \(message)"
        case .invalidContainerIdentifier:
