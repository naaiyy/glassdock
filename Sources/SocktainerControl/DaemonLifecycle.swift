...
struct LaunchAgentPaths: Sendable, Equatable {
    static let label = "io.github.socktainer.daemon"
    ...
    var logDirectory: URL {
        homeDirectory.appendingPathComponent("Library/Logs/Socktainer", isDirectory: true)
    }
    ...
    var socket: URL { homeDirectory.appendingPathComponent(".socktainer/container.sock") }
    var controlLock: URL { homeDirectory.appendingPathComponent(".socktainer/control.lock") }
    var defaultEngineStateDirectory: URL {
        URL(fileURLWithPath: "/private/var/tmp/socktainer-\(Darwin.getuid())/engine", isDirectory: true)
    }
}
...
struct DaemonLifecycle: Sendable {
    ...
    func installationInfo() -> InstallationInfo {
        ...
        if path.hasPrefix("/opt/socktainer/") {
            kind = .package
    ...
    func resolveDaemonExecutable() throws -> URL {
        let candidates = [
            currentExecutable.deletingLastPathComponent().appendingPathComponent("socktainer"),
            URL(fileURLWithPath: "/opt/socktainer/bin/socktainer"),
            URL(fileURLWithPath: "/opt/homebrew/bin/socktainer"),
            URL(fileURLWithPath: "/usr/local/bin/socktainer"),
        ]
        let fileManager = FileManager.default
        guard
...
enum DaemonSafety {
    static func validate(
        ...
        switch intent {
        case .start:
            return reachable ? .noChange("Socktainer is already running.") : .proceed
        case .stop:
            if !managed {
                guard reachable else { return .noChange("Socktainer is already stopped.") }
                throw ControlError.lifecycle(
                    "The running daemon is not managed by Socktainer Control. Stop its foreground process instead."
                )
            }
        case .restart:
            if !managed, reachable {
                throw ControlError.lifecycle(
                    "The running daemon is not managed by Socktainer Control. Stop its foreground process before you start a managed daemon."
                )
            }
        }
