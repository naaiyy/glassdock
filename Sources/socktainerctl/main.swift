...
import Foundation
import SocktainerControl

@main
struct SocktainerControlCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "socktainerctl",
        abstract: "Control the local Socktainer daemon and containers.",
...
private struct Status: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        print("Socktainer: \(snapshot.daemon.state.rawValue)")
    ...
    struct List: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            guard snapshot.daemon.healthy else {
                throw ValidationError(snapshot.daemon.message ?? "Socktainer is not available.")
            }
            if json {
